...
[Hardhat](https://hardhat.org/) is a development environment for Ethereum that helps developers manage and automate the common tasks involved in building smart contracts and decentralized applications. It can directly interact with <EnvText envKey="networkName"/>'s Ethereum API, allowing for the deployment of smart contracts into the <EnvText envKey="networkName"/>.
...
In this example, let's deploy a simple contract in <EnvText envKey="projectName"/> Testnet.

## Prerequisites

Before we start, make sure you have met the following conditions:

1. Have some <EnvText envKey="symbolTestnet"/> on <EnvText envKey="projectName"/> Testnet Network. You can follow our guide on <EnvLink envKey="bridegeDepositUrl">Bridging for <EnvText envKey="networkName"/></EnvLink>.
...
4. Modify the `hardhat.config.js` file
    ...
    - Inside the `module.exports` add the <EnvText envKey="networkName"/> configuration
    ```
    ...
    networks: {
        ...
        <EnvText envKey="projectName"/>: {
            url: "https://<EnvText envKey="projectName"/>-testnet.alt.technology/",  // <EnvText envKey="networkName"/> RPC URL Here
            chainId: 1637450, // <EnvText envKey="networkName"/>ID Here
        }
    },
    }
    ```
...
4. Deploy `your_contract.sol` using the command below
    ```
    npx hardhat run scripts/deploy.js --network <EnvText envKey="projectName"/>
    ```
